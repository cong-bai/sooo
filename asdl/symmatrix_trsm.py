...
assert inverse == "trsm"
from .utils_trsm import cholesky_inv, cholesky_inv_B, smw_inv
...
class Kron:
    ...
    def update_inv(self, damping=_default_damping, calc_A_inv=True, calc_B_inv=True, eps=1e-7, replace=False):
        ...
        if calc_B_inv:
            ...
            if not torch.all(self.B == 0):
                if self.B_is_square:
                    self.B_inv = cholesky_inv_B(self.B, damping_B)
                else:
    ...
    def mvp(self, vec_weight, vec_bias=None, use_inv=False, inplace=False):
        ...
        mvp_w = torch.cholesky_solve(vec_weight_2d, mat_B)
        mvp_w = torch.mm(mvp_w, mat_A).view_as(vec_weight)
        #mvp_w = mat_B.mm(vec_weight_2d).mm(mat_A).view_as(vec_weight)
        if inplace:
            vec_weight.copy_(mvp_w)
        if vec_bias is not None:
            #print(vec_bias.view(self.B_dim, 1).shape)
            mvp_b = torch.cholesky_solve(vec_bias.view(self.B_dim, 1), mat_B).squeeze()
            # print(mvp_b.shape)
