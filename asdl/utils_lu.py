...
import os
assert os.environ.get('inverse') == "lu"
...
def cholesky_inv(X, damping=1e-7):
    X = X.float()
    # if(np.isnan(X.cpu()).sum()>0):
    #      print('X')
    diag = torch.diagonal(X)
    diag += damping
    u = torch.linalg.inv(X)
    # if(np.isnan(u.cpu()).sum()>0):
    #      print('u')
    #      np.savetxt( "X.csv", X.cpu(), delimiter="," )
    #      np.savetxt( "U.csv", u.cpu(), delimiter="," )
    #      sys.exit(0)
    diag -= damping
    return u
