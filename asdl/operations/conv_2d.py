...
class Conv2d(Operation):
    ...
    def cov_kron_A(module, in_data):
        ...
        m = in_data.transpose(0, 1).flatten(
            start_dim=1
        ).double()  # (c_in)(kernel_size) x n(out_size)
        return torch.matmul(
            m, m.T
        ).double().div(out_size)  # (c_in)(kernel_size) x (c_in)(kernel_size)
    ...
    def cov_kron_B(module, out_grads):
        m = out_grads.transpose(0,
                                1).flatten(start_dim=1).double()  # c_out x n(out_size)
        return torch.matmul(m, m.T).double()  # c_out x c_out
